...
//           }
    ...
    struct servent *results;

    results = getservbyname("badger.8080", "tcp");

    g_assert(results != NULL);

    g_assert_cmpstr(results->s_name, ==, "badger.8080");
    g_assert(results->s_aliases[0] == NULL);
    g_assert_cmpint(results->s_port, ==, 18080);
    g_assert_cmpstr(results->s_proto, ==, "tcp");
